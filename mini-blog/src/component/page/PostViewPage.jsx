...
function PostViewPage(props) {
  ...
  // --- 이 줄을 변경합니다. ---
  const { id } = useParams(); // postId 대신 'id'로 변경
  // --- 여기까지 변경합니다. ---

  console.log("PostViewPage: URL에서 가져온 id =", id); // 로그 메시지도 'id'로 변경
  console.log("PostViewPage: 숫자로 변환된 id =", Number(id)); // 로그 메시지도 'id'로 변경
  ...
  const post = data.find((item) => {
      // --- 이 줄도 변경합니다. ---
      return item.id === Number(id); // Number(postId) 대신 Number(id)로 변경
      // --- 여기까지 변경합니다. ---
  });
